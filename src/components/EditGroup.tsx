  ...
  setup(props, context) {
    ...
    return () => (
      <div class="edit-groups">
        <a-collapse activeKey={currentKey.value} onChange={(e: any) => { currentKey.value = e }}>
          {
            editGroups.value.map((item, index) => (
              <a-collapse-panel key={`item-${index}`} header={item.text}>
                <props-table props={item.props} onChange={handleChange}></props-table>
              </a-collapse-panel>
            ))
          }
